...
from yuruquant.reporting.cost_overlay import apply_cost_overlay, build_platform_cost_report
from yuruquant.reporting.trade_records import build_trade_records


class CostOverlayTest(unittest.TestCase):
    def test_build_platform_cost_report_keeps_local_cost_columns_zero(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = build_platform_cost_report(trades, portfolio_path, config, 'gm_builtin_unified')
        ...
        self.assertAlmostEqual(0.0, result.trade_diagnostics[0]['commission_cost'], places=6)
        self.assertAlmostEqual(0.0, result.trade_diagnostics[0]['slippage_cost'], places=6)
        self.assertAlmostEqual(0.0, result.trade_diagnostics[0]['total_cost'], places=6)
        self.assertAlmostEqual(200.0, result.trade_diagnostics[0]['actual_net_pnl'], places=6)
        self.assertAlmostEqual(0.0, result.summary['commission_cost'], places=6)
        self.assertAlmostEqual(0.0, result.summary['slippage_cost'], places=6)
        self.assertAlmostEqual(0.0, result.summary['total_cost'], places=6)
        self.assertAlmostEqual(200.0, result.summary['net_pnl'], places=6)
        self.assertEqual(1, len(result.portfolio_daily))
        self.assertAlmostEqual(500200.0, result.portfolio_daily[0]['equity_end'], places=6)
        self.assertAlmostEqual(0.0, result.portfolio_daily[0]['cumulative_total_cost'], places=6)
        self.assertEqual(1, len(result.symbol_cost_drag))
        self.assertAlmostEqual(0.0, result.symbol_cost_drag[0]['total_cost'], places=6)
