...
def armed_risk_base_equity(config: AppConfig, runtime: RuntimeState) -> float:
...
def entry_signal_risk_amount(config: AppConfig, csymbol: str, signal: EntrySignal) -> float:
...
def position_risk_amount(config: AppConfig, csymbol: str, position: ManagedPosition) -> float:
    spec = config.universe.instrument_overrides.get(csymbol, config.universe.instrument_defaults)
...
def check_entry_against_armed_risk_cap(config: AppConfig, runtime: RuntimeState, csymbol: str, signal: EntrySignal) -> ArmedRiskCheck:
    ...
    proposed_ratio = entry_signal_risk_amount(config, csymbol, signal) / equity
