...
from yuruquant.portfolio.risk_schedule import resolve_drawdown_risk_schedule
...
def evaluate_portfolio_guard(
    ...
    drawdown_schedule = resolve_drawdown_risk_schedule(state.drawdown_ratio, drawdown_risk_schedule)
    ...
    if drawdown_halt_mode == 'hard' and state.drawdown_ratio >= max_drawdown_halt_ratio:
    ...
    if drawdown_schedule.risk_mult < 1.0:
        state.risk_state = "throttle_drawdown"
        state.effective_risk_mult = drawdown_schedule.risk_mult
        state.halt_flag = False
        state.halt_reason = ""
        return GuardDecision(allow_entries=True, force_flatten=False)
