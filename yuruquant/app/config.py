...
class EntryConfig:
    ...
    breakout_close_position_min: float
...
DEFAULTS: dict[str, Any] = {
    ...
    "strategy": {
        ...
        "exit": {
            "hard_stop_atr": 2.2,
            "protected_activate_r": 1.5,
            "trend_ride_activate_r": 2.5,
            "trailing_ma_period": 60,
        },
    },
    "portfolio": {
...
ENTRY_KEYS = {"donchian_lookback", "min_channel_width_atr", "breakout_atr_buffer", "breakout_close_position_min"}
EXIT_KEYS = {"hard_stop_atr", "protected_activate_r", "trend_ride_activate_r", "trailing_ma_period"}
...
def load_config(path: str | Path) -> AppConfig:
    ...
    close_position_min = float(entry["breakout_close_position_min"])
    if not 0 < close_position_min < 1:
        raise ValueError("strategy.entry.breakout_close_position_min must satisfy 0 < x < 1")
    if float(exit_cfg["hard_stop_atr"]) <= 0:
        raise ValueError("strategy.exit.hard_stop_atr must be > 0")
    if int(exit_cfg["trailing_ma_period"]) < 2:
        raise ValueError("strategy.exit.trailing_ma_period must be >= 2")
    ...
    return AppConfig(
        ...
        strategy=StrategyConfig(
            ...
            entry=EntryConfig(
                ...
                breakout_close_position_min=float(entry["breakout_close_position_min"]),
            ),
            exit=ExitConfig(
                hard_stop_atr=max(float(exit_cfg["hard_stop_atr"]), 0.01),
                protected_activate_r=max(float(exit_cfg["protected_activate_r"]), 0.0),
                trend_ride_activate_r=max(float(exit_cfg["trend_ride_activate_r"]), 0.0),
                trailing_ma_period=max(int(exit_cfg["trailing_ma_period"]), 2),
            ),
        ),
        portfolio=PortfolioConfig(
